...
const NavMenu = () => {
  ...
  // <-- KROK 2: Dodaj ten blok useEffect
  // Ten hook będzie obserwował zmiany w `isMobileMenuOpen`.
  useEffect(() => {
    // Jeśli menu mobilne zostało właśnie zamknięte (stan zmienił się na `false`)...
    if (!isMobileMenuOpen) {
      // ...zresetuj stan aktywnego submenu.
      setActiveMenu(null);
    }
  }, [isMobileMenuOpen]); // Tablica zależności - kod uruchomi się tylko, gdy `isMobileMenuOpen` się zmieni.
  ...
  const renderMenuItem = (icon, title, path, submenuKey, submenuItems) => {
    const finalIcon = React.cloneElement(icon, {
      ...
      style: {
        transform:
          window.innerWidth > 768 ? `rotate(${iconRotation}deg)` : "none",
        transition: "transform 0.8s",
      },
    });
  ...
  return (
    <>
      ...
      {isMobileMenuOpen && (
        <div className="nav-overlay" onClick={handleMobileMenuClose}></div>
      )}

      <ul
        className={`navmenu__container ${collapsed ? "collapsed" : ""} ${
